...
function renderPublications(){
  ...
  buildSelect("#pub-year-filter", years, "All years");
  buildSelect("#pub-year-start-filter", years, "From year");
  buildSelect("#pub-year-end-filter", years, "To year");
  ...
  const q = ($("#pub-search") ? $("#pub-search").value : "").toLowerCase();

  const items = all.filter(p => {
    const tagsText = pubFilterText(p);
    const text = [
      arrayText(p.authors),
      arrayText(p.authors_en),
      arrayText(p.authors_zh),
      p.title,
      p.title_en,
      p.title_zh,
      p.title_cn,
