...
def ensure_clean_repo() -> None:
    ...
    allowed = {
        str(DB_PATH.relative_to(REPO_ROOT)).replace("\\", "/"),
        str(WEB_JSON_PATH.relative_to(REPO_ROOT)).replace("\\", "/"),
    }
    blocking: list[str] = []
    ...
    if blocking:
        raise SystemExit(
            "Git working tree has uncommitted code/config changes. Commit/stash them first, then rerun update_papers.bat.\n"
            + "\n".join(blocking)
        )
...
def publish(branch: str) -> None:
    ...
    files = [str(DB_PATH.relative_to(REPO_ROOT)), str(WEB_JSON_PATH.relative_to(REPO_ROOT))]
    run(["git", "add", *files])
    ...
    run(["git", "commit", "-m", "chore: update online paper monitor"])
    ...
    with tempfile.TemporaryDirectory(prefix="paper-monitor-") as td:
        td = Path(td)
        db_copy, json_copy = td / "papers.db", td / "online_papers.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="V6 local ID-first updater")
    ...
    parser.add_argument("--no-push", action="store_true", help="pull first, fetch/export, leave generated files uncommitted")
