...
def load_journal_list(path: str | Path | None = None) -> tuple[JournalSpec, ...]:
    ...
    for row in rows:
        ...
        specs.append(JournalSpec(
            ...
            crossref_member=_int_or_none(val(row, "Crossref Member", "CrossrefMember", "Member")),
            crossref_prefix=normalize_space(str(val(row, "Crossref Prefix", "CrossrefPrefix", "DOI Prefix"))),
            crossref_group=normalize_space(str(val(row, "Crossref Group", "CrossrefGroup"))),
        ))
    return tuple(specs)
...
def match_journal(provider: str, journal: str | None, issn: str | None, specs: Sequence[JournalSpec] | None = None) -> JournalSpec | None:
    ...
    if tk:
        for spec in candidates:
            if tk in {_title_key(n) for n in (spec.journal, *spec.aliases) if n}:
                return spec
    return None
