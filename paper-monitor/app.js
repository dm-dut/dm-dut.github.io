...
async function triggerRefresh() {
  ...
  // Actions page instead of showing a configuration error.
  const actionsUrl = String(CONFIG.actionsUrl || "").trim();
  if (actionsUrl) {
    window.open(actionsUrl, "_blank", "noopener,noreferrer");
    setRefreshState("working", "已打开 GitHub Actions。运行 “Update paper monitor” 后，返回本页刷新即可。");
    return;
  }
  setRefreshState("error", "未配置安全更新入口或 GitHub Actions 地址。");
}
